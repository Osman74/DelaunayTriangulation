...
public:
  ...
  BoundingBox3d(const Point3 &pt) : MinPt(pt), MaxPt(pt) {}
  BoundingBox3d(double xmin, double ymin, double zmin, double xmax,
  ...
  void reset()
  {
    MinPt = Point3(DBL_MAX, DBL_MAX, DBL_MAX);
    MaxPt = Point3(-DBL_MAX, -DBL_MAX, -DBL_MAX);
  }
  ...
  Point3 min() const { return MinPt; }
  Point3 max() const { return MaxPt; }
  Point3 center() const { return (MinPt + MaxPt) * .5; }
  double diag() const { return MinPt.distance(MaxPt); }
  void makeCube()
  {
    Vector3 len = MaxPt - MinPt;
    Point3 cc = center();
    MaxPt = cc + Point3(1, 1, 1);
    MinPt = cc + Point3(-1, -1, -1);
