...
bool Triangulation::visibilityTest(Node p_Node, FiniteElement p_FE)
{
	Node* A = m_Nodes[(p_FE.m_NodeIDs[0]-1)];
	Node* B = m_Nodes[p_FE.m_NodeIDs[1]-1];
	Node* C = m_Nodes[p_FE.m_NodeIDs[2]-1];
	std::vector<double> AB = vector(*A, *B);
	std::vector<double> BC = vector(*B, *C);
	std::vector<double> n = crossProduct(AB, BC);
	std::vector<double> AP = vector(*A, p_Node);
	return scalarProduct(n, AP) > 0;
}

std::ostream& operator<<(std::ostream& p_Out, Triangulation& p_Triangulation)
{
	for (auto tetra = p_Triangulation.m_Tetrahedrons.begin(); tetra != p_Triangulation.m_Tetrahedrons.end(); tetra++)
	{
		p_Out << "Тетраэдр " << std::endl;
		p_Out << **tetra;
	}
	return p_Out;
}
...
Triangulation::~Triangulation()
{
    for (auto it = m_Nodes.begin(); it < m_Nodes.end(); ++it)
        delete* it;
    for (auto it = m_Tetrahedrons.begin(); it < m_Tetrahedrons.end(); ++it)
        delete* it;
}
...
struct Tet {
    ...
    {
        _modified = true;
        double val = orient3d((double*)v0, (double*)v1,
            (double*)v2, (double*)v3);
        V[0] = v0;
        V[1] = v1;
        V[2] = v2;
...
public:
    ...
    tetContainer(int nbThreads, int pdoublelocSizePerThread)
    {
        _perThread.resize(nbThreads);
        for (std::size_t i = 0; i < _perThread.size(); i++) {
            _perThread[i] = new aBunchOfStuff<Tet>(pdoublelocSizePerThread);
        }
    }
...
#pragma omp barrier
    ...
    if (invalidCavities[0]) 0//Msg::Error("%d invalid cavities", invalidCavities[0]);
...
static void delaunayTriang(const int numThreads, const int nptsatonce,
...
double exactinit(int filter, double maxx, double maxy, double maxz)
{
  ...
  /*  cword = 4735; */
    ...
    double iccerrboundB = (4.0 + 48.0 * epsilon) * epsilon;
    double iccerrboundC = (44.0 + 576.0 * epsilon) * epsilon * epsilon;
    ...
    double _use_inexact_arith = 0;
    double _use_static_filter = filter;
    ...
    double o3dstaticfilter = 5.1107127829973299e-15 * maxx * maxy * maxz;
    double ispstaticfilter = 1.2466136531027298e-13 * maxx * maxy * maxz * (maxz * maxz);

    return epsilon; /* Added by H. Si 30 Juli, 2004. */
}

void Triangulation::delaunayTriangulation(const int numThreads, const int nptsatonce,
    bool removeBox)
{
    std::vector<Node*> _temp;
    ...
    tetContainer allocator(numThreads, m_Nodes.size() * 10);
    ...
    exactinit(1, maxx, maxy, maxz);

    Vert* box[8];
    delaunayTriang(numThreads, nptsatonce, _vertices, box, allocator);
    // print("finalTetrahedrization.pos",0, allocator);
    ...
    for (int i = 0; i < 8; i++) {
        ...
        if (removeBox) { v->setNum(0); }
            ...
            v->setNum(N + i + 1);
            Node* mv = new Node(N + (i + 1), v->x(), v->y(), v->z());
            _temp[v->getNum()] = mv;
            m_Nodes.push_back(mv);
        }
    }
        ...
        for (std::size_t i = 0; i < allocator.size(myThread); i++) {
            ...
            if (t->V[0]) {
                if (t->V[0]->getNum() && t->V[1]->getNum() && t->V[2]->getNum() &&
                    t->V[3]->getNum()) {
                    Node* v1 = _temp[t->V[0]->getNum()];
                    Node* v2 = _temp[t->V[1]->getNum()];
                    Node* v3 = _temp[t->V[2]->getNum()];
                    Node* v4 = _temp[t->V[3]->getNum()];
                    Tetrahedron* tr = new Tetrahedron(*v1, *v2, *v3, *v4);
                    m_Tetrahedrons.push_back(tr);
                }
                else if (!removeBox) {
                    ;
                }
...
void Triangulation::inputNodes(std::string fileName)
{
    char buff[50];
	...
	for (int i = 0; i < numNodes; ++i)
	{
        double x, y, z;
        fin >> x >> y >> z;
        Node* tmp = new Node(i + 1, x, y, z);
        m_Nodes.push_back(tmp);
	}
}

void Triangulation::generateMesh(std::string fileName)
{
    delaunayTriangulation(1, 1, true);
    std::ofstream fout;
    fout.open(fileName);
    ...
    fout << numNodes << std::endl;
    int i = 1;
    for (auto it = m_Nodes.begin(); it < m_Nodes.end(); ++it)
    {
        fout << i << " " << (*it)->x() << " " << (*it)->y() << " " << (*it)->z() << std::endl;
        ++i;
    }
    fout << "$EndNodes" << std::endl;
    fout << "$Elements" << std::endl;
    int numElements = m_Tetrahedrons.size();
    fout << numElements << std::endl;
    i = 0;
    for (auto it = m_Tetrahedrons.begin(); it < m_Tetrahedrons.end(); ++it)
    {
        fout << i << " 4" << " 0 " << **it << std::endl;
        ++i;
    }
